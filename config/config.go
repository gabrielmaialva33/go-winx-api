...
type config struct {
	...
	LogChannelID   int64        `envconfig:"LOG_CHANNEL_ID" required:"true"`
...
func SetFlagsFromConfig(cmd *cobra.Command) {
	...
	cmd.Flags().Int64("channel-id", ValueOf.ChannelID, "Telegram Channel ID")
	...
	cmd.Flags().StringSlice("string-sessions", ValueOf.StringSessions, "Pyrogram string sessions")
}
...
func (c *config) loadConfigFromArgs(log *zap.Logger, cmd *cobra.Command) {
	...
	channelID, _ := cmd.Flags().GetInt64("channel-id")
	if channelID != 0 {
		os.Setenv("CHANNEL_ID", strconv.FormatInt(channelID, 10))
	}
	logChannelID, _ := cmd.Flags().GetString("log-channel")
	if logChannelID != "" {
		os.Setenv("LOG_CHANNEL_ID", logChannelID)
	}
	...
	stringSessions, _ := cmd.Flags().GetStringSlice("string-sessions")
	if len(stringSessions) > 0 {
		os.Setenv("STRING_SESSIONS", strings.Join(stringSessions, ","))
	}
}
